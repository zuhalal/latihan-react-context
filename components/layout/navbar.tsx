...
import { MovieType, RandomUser } from '../../models'
import { MenuCustom } from '../menu'

export const Navbar = () => {
  const [data, setData] = useState({} as RandomUser)
  const [movies, setMovies] = useState({} as MovieType)

  // to do, get first name from https://randomuser.me/api
  useEffect(() => {
  ...
  //implement context so that my favorite movies same in home
  ...
  return (
    ...
    <div className="container flex flex-wrap justify-between items-center mx-auto">
      <div className="flex gap-3">
        <p>Hello, {data.results?.length > 0 && data.results[0]?.name?.first}</p>
